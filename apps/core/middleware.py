        ...
        for key, val in ids_to_check:
            if key in ID_MODEL_MAPPING and val:
                ...
                try:
                    ...
                    elif hasattr(obj, 'barbershop') and hasattr(obj.barbershop, 'organization_id'):
                    ...
                    if obj_org_id is not None and str(obj_org_id) != str(user_org.id):
                        logger.warning(f"INTRUSION ATTEMPT: User {request.user.id} (Org {user_org.id}) tried to access {model_name} {obj_id} (Org {obj_org_id})")
                        return HttpResponseForbidden("Acceso denegado: Violación de límites de tenant.")
...
class GlobalRateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if any(path.startswith(p) for p in ['/accounts/login/', '/accounts/signup/', '/accounts/password/']):
            if is_ratelimited(request, group='auth', key='ip', rate='5/m', increment=True):
                return JsonResponse({"error": "Demasiadas peticiones. Intente nuevamente en un minuto."}, status=429)
                
        # 2. Formularios Públicos / Booking / Contacto (10/m)
        elif path.startswith('/booking/') or 'contact' in path:
            if is_ratelimited(request, group='public_forms', key='ip', rate='10/m', increment=True):
                return JsonResponse({"error": "Demasiadas peticiones desde esta IP."}, status=429)
                
        # 3. Webhooks de Pago
        elif path.startswith('/billing/webhook/'):
            # Los proveedores pueden enviar ráfagas; un límite razonable como 30/m por IP
            if is_ratelimited(request, group='webhooks', key='ip', rate='30/m', increment=True):
                return JsonResponse({"error": "Demasiadas peticiones de webhooks."}, status=429)
