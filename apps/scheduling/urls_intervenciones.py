...
urlpatterns = [
    ...
    path(
        "api/<int:pk>/editar/",
        views.IntervencionUpdateView.as_view(),
        name="api_update",
    ),
    path(
        "api/<int:pk>/eliminar/",
        views.IntervencionDeleteView.as_view(),
        name="api_delete",
    ),
    path(
        "api/<int:pk>/estado/",
        views.IntervencionChangeStatusAPI.as_view(),
        name="api_change_status",
    ),
    path(
        "api/<int:pk>/assign-client/",
        views.IntervencionAssignClientAPI.as_view(),
        name="api_assign_client",
    ),
    # Barber services for the intervenciones modal (barber → services selector)
    path(
        "api/barber-services/<int:barber_id>/",
        scheduling_views.BarberServicesAPI.as_view(),
